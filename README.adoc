...
image::nodeclipse.png[Ecliplse Marketplace]
...
WARNING: In some cases, the initial build will not work correctly, due to a bug that we hope to remedy shortly. You will know this from red error markers
...
+
image::create-gentarget.png[Crate GenTarget]
...
* The handler is expected return a new `Promise` that has either been `resolve`d to a value for the
   response payload, or `reject`ed with an error object that should have `code` and `message`
   properties.
...
+
image::launch-app.png[Launch the Service App]
+
